...
type IRepositoryUser interface {
	GetByID(id string) (user *entity.EntityUser, err error)
	...
	GetUsersFromIDs(ids []string) (users []entity.EntityUser, err error)
	GetUser(id string) (user *entity.EntityUser, err error)
}
...
type IUsecaseUser interface {
	...
	UpdatePassword(id string, oldPassword, newPassword, confirmPassword string) error
